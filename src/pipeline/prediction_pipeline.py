...
class PredictionPipeline:
    ...
    def save_input_files(self)-> str:
        try:
            ...
            prediction_file_path = os.path.join(pred_file_input_dir, input_csv_file.filename)
    ...
    def get_predicted_dataframe(self, input_dataframe_path: pd.DataFrame)-> pd.DataFrame:
        try:
            prediction_column_name: str = TARGET_COLUMN
            input_dataframe = pd.read_csv(input_dataframe_path)

            input_dataframe.drop(columns=["Unnamed: 0"], inplace=True) if "Unnamed: 0" in input_dataframe.columns else input_dataframe
